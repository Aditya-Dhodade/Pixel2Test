...
# def clear_old_files(directory, extension):
#     """Remove all files with a given extension from the directory."""
...
#                                                         try:
#                                                             clear_old_files(FEATURE_FILE_PATH, ".feature")
#                                                             clear_old_files(STEP_PATH, ".py")
#                                                             feature_filename = f"{selected_id}_feature_file.feature"
#                                                             step_filename = f"{selected_id}_step_definition.py"
#                                                             # Save feature file and step definition for execution
#                                                             with open(os.path.join(FEATURE_FILE_PATH, feature_filename), "w") as f:
...
st.markdown("""
    <style>
        ...
        .edit-prompt {
            color: #333333; /* Dark gray for edit prompt */
            margin-bottom: 8px;
        }
        .error-box {
...
if 'uploaded_images' not in st.session_state:
    st.session_state.uploaded_images = []
if 'joining_option' not in st.session_state:
    st.session_state.joining_option = None
if 'delay_option' not in st.session_state:
    st.session_state.delay_option = "x"  # default value
# Initialize GROQ client
client = Groq(api_key="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")
...
def generate_feature_and_step_definition(test_case, selected_id, xpaths=None):
    print("its called")
    delay_instructions = {
        "low": "Add minimal delays (0.5-1 second) between actions",
        "medium": "Add moderate delays (2-3 seconds) between actions",
        "high": "Add significant delays (4-5 seconds) between actions"
    }
    ...
    Above given is a test case scenario based on a form. Generate the feature file corresponding to the given test case and its Python step definition script.
    This is the website for testing {st.session_state.website_url} Use this website in the step definition file.
    {delay_instructions[st.session_state.delay_option]}. 
    Add explicit waits (time.sleep) between each action according to the selected delay level.
    """
    if xpaths:
        prompt += f"\n\nHere are the XPaths of the elements on the website:\n{xpaths}"
...
def show_sidebar():
    """Displays the sidebar with project information and instructions"""
    with st.sidebar:
        st.markdown("""
            <div style="text-align: center;">
                <h2>🧪 AI Test Case Generator</h2>
                <p>Generate automated test cases from UI screenshots</p>
            </div>
        """, unsafe_allow_html=True)
        ...
        2. Enter website URL (optional)
        3. Generate test cases
        4. Select a test case
        5. Generate BDD files
        6. Run tests
        """)
...
show_sidebar()
st.markdown("""
        <h1 class="header">AI-Powered Test Case Generator</h1>
        <p>Automatically generate test cases from UI screenshots and website URLs</p>
    """, unsafe_allow_html=True)
# Upload image section
upload_image()
# taking input of website
st.markdown("### 🌐 Website URL")
st.session_state.website_url = st.text_input("Enter the website URL (Environement):", placeholder="https://example.com",
    help="Enter the URL of the website you want to test")

# Add delay selection dropdown
...
if st.session_state.website_url == "None":
    ...
    if st.session_state.num_test_cases != 0:
        with st.expander("Generate testcases", expanded=True):
        # Ensure images are uploaded before continuing
            if st.session_state.uploaded_images:
                num_images = len(st.session_state.uploaded_images)
                joined_image = None
                ...
                if st.session_state.joining_option != "select the joining method" or num_images == 1:
                    ...
                    with st.spinner("Generating test cases..."):
                        with st.container():
                            ...
                            st.markdown("<h3 style='margin-top: 0;'>Generated Test Cases:</h3>", unsafe_allow_html=True)
                            ...
                            if isinstance(st.session_state.test_cases, list):
                                ...
                                if st.button("🛠 Generate Feature File and Step Definition"):
                                    ...
                                    if st.session_state.selected_test_case:
                                        ...
                                        st.markdown("""
                                            <div class="error-box">
                                                ❌ No test case selected. Please view test case details first.
                                            </div>
                                        """, unsafe_allow_html=True)
                                    if st.session_state.feature_file != "" and st.session_state.step_definition != "":
                                        ...
                                        st.markdown("""
                                            <div class="step-definition">
                                                <p class="edit-prompt">Edit the code below if needed:</p>
                                        """, unsafe_allow_html=True)
                                        st.session_state.edited_code = st.text_area(
                                ...
                                st.markdown("""
                                    <div class="error-box">
                                        ❌ Failed to parse test cases. Ensure the response is in JSON format.
                                    </div>
                                """, unsafe_allow_html=True)
                else:
                    st.markdown("""
                    <div class="info-box">
                        ℹ Please upload at least one UI image to generate test cases.
                    </div>
                """, unsafe_allow_html=True)
    ...
    print(st.session_state.website_url)
    st.markdown("### 🔍 XPath Extraction")
    with st.spinner("Extracting xpath from "  + str(st.session_state.website_url)):
        st.session_state.extracted_xpath = extract_xpaths(st.session_state.website_url)
    if st.session_state.extracted_xpath != {}:
        # Show preview of extracted XPaths
        # if st.session_state.extracted_xpath != {}:
        st.success("XPaths extracted successfully!")
        with st.expander("View Extracted XPaths", expanded=False):
            st.json(st.session_state.extracted_xpath)
    # st.write(st.session_state.extracted_xpath)
    st.markdown("### 🧪 Test Case Generation")
    st.session_state.num_test_cases = st.number_input("Enter the number of test cases to generate:",  step=1)
    if st.session_state.num_test_cases != 0:
                ...
                if st.session_state.joining_option != "select the joining method" or num_images == 1:
                    ...
                    with st.spinner("Generating test cases..."):
                        with st.container():
                            ...
                            st.markdown("<div class='response-container'>", unsafe_allow_html=True)
                            st.markdown("<h3 style='margin-top: 0;'>Generated Test Cases:</h3>", unsafe_allow_html=True)
                            st.markdown(st.session_state.html_table, unsafe_allow_html=True)
                            st.markdown("</div>", unsafe_allow_html=True)
                            ...
                            if isinstance(st.session_state.test_cases, list):
                                ...
                                st.markdown("### ⏱ Action Delay")
                                st.session_state.delay_option = st.selectbox(
                                    "Select delay between actions:",
                                    ["low", "medium", "high"],
                                    help="Select how much delay to add between actions in the test execution"
                                )

                                # Button to generate feature file and step definition
                                if st.button("🛠 Generate Feature File and Step Definition") and st.session_state.delay_option != "x":
                                        ...
                                        try:
                                            ...
                                            if result.returncode == 0:
                                                st.markdown("""
                                                    ...
                                                    """, unsafe_allow_html=True)
                                                st.text_area("Test Output:", result.stdout, height=300)
                                            else:
                                                st.markdown("""
                                                        <div class="error-box">
                    ...
                    st.markdown("""
                        <div class="error-box">
                            ❌ Please select how to join the images before generating test cases.
                        </div>
                    """, unsafe_allow_html=True)
            else:
                st.markdown("""
